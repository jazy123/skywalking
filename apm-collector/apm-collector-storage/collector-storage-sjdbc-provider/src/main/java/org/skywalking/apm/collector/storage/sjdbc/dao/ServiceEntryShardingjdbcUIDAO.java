 ...
 * @author linjiaqi
 */
public class ServiceEntryShardingjdbcUIDAO extends ShardingjdbcDAO implements IServiceEntryUIDAO {

    private final Logger logger = LoggerFactory.getLogger(ServiceEntryShardingjdbcUIDAO.class);
    private static final String GET_SERVICE_ENTRY_SQL = "select * from {0} where {1} >= ? and {2} <= ?";

    public ServiceEntryShardingjdbcUIDAO(ShardingjdbcClient client) {
        super(client);
    }
