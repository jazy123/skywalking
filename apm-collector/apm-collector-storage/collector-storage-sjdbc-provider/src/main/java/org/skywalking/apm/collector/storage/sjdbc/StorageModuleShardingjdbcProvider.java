...
import org.skywalking.apm.collector.storage.sjdbc.strategy.ShardingjdbcStrategy;
...
public class StorageModuleShardingjdbcProvider extends ModuleProvider {
    ...
    public void prepare(Properties config) throws ServiceNotProvidedException {
        Map<String, ShardingNode> shardingNodes = createShardingNode(config);
        ShardingRuleConfiguration shardingRuleConfig = createShardingRule(shardingNodes.size());
        shardingjdbcClient = new ShardingjdbcClient(shardingNodes, shardingRuleConfig);
    ...
    private Map<String, ShardingNode> createShardingNode(Properties config) {
        Map<String, ShardingNode> shardingNodes = new HashMap<String, ShardingNode>();
        int index = 0;
        while (true) {
            String url = config.getProperty(URL + Const.ID_SPLIT + index);
            String username = config.getProperty(USER_NAME + Const.ID_SPLIT + index);
            String password = config.getProperty(PASSWORD + Const.ID_SPLIT + index);
            if (url != null && username != null && password != null) {
                shardingNodes.put(ShardingjdbcStrategy.SHARDING_DS_PREFIX + index, new ShardingNode(url, username, password));
                index++;
            } else {
                break;
            }
